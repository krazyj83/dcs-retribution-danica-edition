"""
qt/logisticspanel.py

Qt widget for the Logistics module.
Provides three tabs:
  1. Drop Zones  — create/edit/delete troop and cargo drop zones on the map
  2. Warehouses  — view stock levels, run import/export between bases
  3. Transfers   — schedule, view, and cancel logistics deliveries

This panel is added to the main window tab bar alongside the existing
Ato, Intelligence, and Settings panels.

Concept — Qt signals used here:
  dropZoneAdded(DropZone)    → tells the map canvas to draw the new zone
  dropZoneRemoved(str)       → tells the map canvas to remove zone by dz_id
  transferScheduled(...)     → triggers mission generator to note new transfer
"""
...
# PySide6 is the Qt binding used by dcs-retribution
...
from game.logistics import (
    DropZone, DropZoneType, Warehouse, WarehouseCategory,
...
# ======================================================================
# Drop Zone creation dialog
# ======================================================================

class DropZoneDialog(QDialog):
    """
    Modal dialog for creating or editing a DropZone.
    The player fills in name, type, lat/lon, radius, and notes.
    In a future iteration lat/lon will be clickable from the map canvas.
    """
    ...
    def get_drop_zone(self) -> DropZone:
        """Build and return the DropZone from current dialog state."""
        dz_id = self.existing.dz_id if self.existing else None
        kwargs = dict(
...
class DropZonesTab(QWidget):
    """Lists all drop zones for a coalition, with add/edit/delete."""
    ...
    def _build_ui(self) -> None:
        ...
        self.info_label = QLabel(
            ...
            "Lua scripts (MOOSE CTLD) use them for troop and cargo delivery."
        )
        self.info_label.setWordWrap(True)
        self.info_label.setStyleSheet("color: grey; font-size: 11px;")
    ...
    def refresh(self) -> None:
        ...
        for row, dz in enumerate(dzs):
            ...
            # Store dz_id as item data on the name cell
    ...
    def _on_add(self) -> None:
        # In a full implementation, the cp_id/cp_name come from a
        # base-selector combo. Here we use a placeholder.
        dlg = DropZoneDialog(cp_id=0, cp_name="Selected Base", coalition=self.coalition, parent=self)
...
class WarehouseTab(QWidget):
    """
    Shows stock levels for all coalition bases.
    Allows exporting supply from one base to another directly
    (as an instant transfer used before the campaign starts,
     or as a manual balance for the current turn).
    """
    ...
    def _build_ui(self) -> None:
        ...
        # Warehouse table (one row per CP, columns per category)
        self.table = QTableWidget()
        ...
        # Export panel
        export_group = QGroupBox("Export / Transfer Stock")
    ...
    def refresh(self) -> None:
        ...
        for row, wh in enumerate(warehouses):
            ...
            for col, cat in enumerate(cats, start=1):
                ...
                pct = int(100 * item_data.quantity / item_data.capacity) if item_data.capacity else 0
                text = f"{item_data.quantity:.0f} / {item_data.capacity:.0f} ({pct}%)"
    ...
    def _on_export(self) -> None:
        ...
        if from_cp_id == to_cp_id:
            ...
            return
        if amount <= 0:
            self.status_label.setText("⚠ Amount must be > 0.")
            return
...
class TransfersTab(QWidget):
    """
    Schedule and monitor logistics transfers (helo / transport missions).
    """
    ...
    def refresh(self) -> None:
        # Repopulate source / destination combos
        warehouses = self.logistics.warehouses_for_coalition(self.coalition)
        ...
        for row, t in enumerate(all_transfers):
            ...
            self.table.setItem(row, 1, QTableWidgetItem(src_wh.cp_name if src_wh else str(t.source_cp_id)))
            self.table.setItem(row, 2, QTableWidgetItem(dst_wh.cp_name if dst_wh else str(t.dest_cp_id)))
    ...
    def _on_schedule(self) -> None:
        ...
        if src_cp_id == dst_cp_id:
            ...
            return
        if not dz_id:
            self.sched_status.setText("⚠ No drop zone available at destination.")
            return
        ...
        if transfer is None:
            self.sched_status.setText("⚠ Insufficient stock at source warehouse.")
        else:
...
# ======================================================================
# Main logistics panel (tab widget)
# ======================================================================

class LogisticsPanel(QWidget):
    """
    Top-level logistics panel.
    Add to MainWindow with:
        self.tab_widget.addTab(LogisticsPanel(game), "Logistics")
    """

    def __init__(self, game, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self.game = game
        logistics: LogisticsManager = game.logistics
        coalition = "blue"  # Player always plays blue in single-player
    ...
    def refresh(self) -> None:
        """Call after each turn to refresh all sub-tabs."""
        self.dz_tab.refresh()
        self.wh_tab.refresh()
        self.tr_tab.refresh()
